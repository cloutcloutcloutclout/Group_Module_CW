...
<head>
    ...
    <style>
        ...
        @media (min-width: 900px) {
            .course-section { padding-left: 230px; padding-right: 20px; }
        }
        .course-card { transition: transform 0.3s ease, box-shadow 0.3s ease; }
        .course-card:hover { transform: translateY(-6px); box-shadow: 0 6px 18px rgba(0,0,0,0.2); }
    </style>
</head>
<body>
...
<section class="course-section">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <!-- Add New Course button at top right -->
            <a href="${pageContext.request.contextPath}/admin/courses/new"
            ...
            <c:forEach var="course" items="${courses}">
                <div class="col-12 col-md-6 col-lg-4 col-xl-3">
                    <div class="card shadow-sm course-card h-100">
                        <img src="${course.image}" class="card-img-top" style="aspect-ratio:1/1; object-fit:contain; background:white;">
                        <div class="card-body d-flex flex-column">
                            <h5 class="text-primary text-center fw-bold">${course.name}</h5>
                            <p class="text-muted small text-center">${course.description}</p>
                            <p class="text-muted small text-center"><i class="fa-regular fa-clock"></i> ${course.durationMinutes} mins</p>

                            <div class="mt-auto">
                                <a href="${pageContext.request.contextPath}/admin/courses/edit/${course.id}" class="btn btn-primary w-100 mb-2">Edit</a>

                                <form action="${pageContext.request.contextPath}/admin/courses/delete/${course.id}" method="post">
                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
                                    <button type="submit" class="btn btn-danger w-100" onclick="return confirm('Delete this course?');">Delete</button>
                                </form>

                                <a href="${pageContext.request.contextPath}/admin/reviews/${course.id}" class="btn btn-info btn-sm mt-2">Manage Reviews</a>
                            </div>
                        </div>
                    </div>
                </div>
