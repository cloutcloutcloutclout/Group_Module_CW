...
<head>
    ...
    <style>
        ...
        .add-btn:hover { background: #7b92d6; }
...
<section>
    <div class="page-content">
        ...
        <h1><i class="fa-solid fa-user-plus me-2"></i>Find Friends</h1>
                ...
                <c:forEach var="req" items="${incomingRequests}">
                    <div class="friend-row">
                        ...
                        <form action="${pageContext.request.contextPath}/friend/accept/${req.requester.id}" method="post">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
                            <button class="add-btn" type="submit">Accept</button>
                        </form>
        ...
        <c:choose>
            <c:when test="${empty users}">
                <p class="empty-text">No new people to add.</p>
            </c:when>
            <c:otherwise>
                <c:forEach var="u" items="${users}">
                    <div class="friend-row">
                        <c:choose>
                            <c:when test="${not empty u.userProfile}">
                                <img src="${u.userProfile.avatar}" alt="avatar"/>
                                <div class="friend-info">
                                    <span>${u.userProfile.firstName} ${u.userProfile.lastName}</span>
                                    <p>${u.userProfile.bio}</p>
                                </div>
                            </c:when>
                            <c:otherwise>
                                <img src="https://static.vecteezy.com/system/resources/previews/024/766/958/non_2x/default-male-avatar-profile-icon-social-media-user-free-vector.jpg" alt="avatar"/>
                        ...
                        <form action="${pageContext.request.contextPath}/friend/request/${u.id}" method="post">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
...
<script>
    ...
    /* script for dropdown menu, navbar */
